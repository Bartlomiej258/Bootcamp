...
public class ShopService {
    ...
    public void addItem(Long orderId, Long productId, double qty) {
        orders.stream()
                ...
                .forEach(o -> o.items().add(new Item(productId, qty)));
    }
    ...
    public boolean removeItem(Long orderId, Long productId) {
        ...
        while (orderIterator.hasNext()) {
            ...
            int orderSize = theOrder.items().size();
            for (int n = 0; n < theOrder.items().size(); n++) {
                if (theOrder.items().get(n).productId().equals(productId)) {
                    theOrder.items().remove(n);
                    return true;
                }
            }
        }
