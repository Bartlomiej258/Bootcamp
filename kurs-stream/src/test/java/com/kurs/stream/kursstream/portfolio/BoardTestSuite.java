...
public class BoardTestSuite {
    ...
    void testAddTaskListFindLongTasks() {
        ...
        var longTask = project.getTaskLists().stream()
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        //When
        List<TaskList> averageDays = new ArrayList<>();
        averageDays.add(new TaskList("In progress"));
        double avgDays = project.getTaskLists().stream()
                .filter(averageDays::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .mapToLong(l -> ChronoUnit.DAYS.between(l, LocalDate.now()))
                .average()
                .orElse(0);

        //Then
        assertEquals(10, avgDays, 0.001);
    }
